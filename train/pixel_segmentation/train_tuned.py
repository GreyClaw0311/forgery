...
python train_tuned.py --data_dir /path/to/data --custom
"""
...
class Config:
    ...
    def get_lgb_params(self):
        return {
            ...
            'boosting_type': 'gbdt',
            'num_leaves': self.NUM_LEAVES if hasattr(self, 'NUM_LEAVES') else 127,
            'learning_rate': self.LEARNING_RATE if hasattr(self, 'LEARNING_RATE') else 0.03,
            'max_depth': self.MAX_DEPTH if hasattr(self, 'MAX_DEPTH') else 12,
            'feature_fraction': 0.8,
            'bagging_fraction': 0.8,
            'bagging_freq': 5,
            'verbose': -1,
            'n_jobs': -1,
            'scale_pos_weight': self.SCALE_POS_WEIGHT if hasattr(self, 'SCALE_POS_WEIGHT') else 20,
            'is_unbalance': True,  # 关键: 自动处理不平衡
        }
...
class SmartSampler:
    ...
    def sample(self, features: np.ndarray, labels: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 篡改样本过采样
        if n_tamper_target > n_tamper:
            # 需要过采样
            oversample_times = n_tamper_target // n_tamper
            oversample_remainder = n_tamper_target % n_tamper
            ...
            tampered_sampled = np.random.choice(tampered_idx, n_tamper_target, replace=False)
        
        # 正常样本欠采样
        n_normal_target = int(len(tampered_sampled) * self.config.NORMAL_UNDERSAMPLE_RATIO)
        n_normal_target = min(n_normal_target, len(normal_idx), 
                              self.config.MAX_SAMPLES_PER_IMAGE - len(tampered_sampled))
        
        normal_sampled = np.random.choice(normal_idx, n_normal_target, replace=False)
...
class FastFeatureExtractor:
    ...
    def _compute_lbp_slow(self, gray, radius=1, n_points=8):
        h, w = gray.shape
...
class ProgressTracker:
    ...
    def print_progress(self, completed: int):
        ...
        bar_len = 30
        filled = int(bar_len * completed / self.total) if self.total > 0 else 0
...
def main():
    ...
    parser = argparse.ArgumentParser(description='像素级分割训练 - 调参优化版')
    ...
    print(f"最大采样: {config.MAX_SAMPLES_PER_IMAGE}")
    print(f"篡改过采样: {config.TAMPER_OVERSAMPLE_RATIO}x")
    print(f"正常欠采样: {config.NORMAL_UNDERSAMPLE_RATIO}x")
    print(f"类别权重: {config.SCALE_POS_WEIGHT if hasattr(config, 'SCALE_POS_WEIGHT') else 'auto'}")
